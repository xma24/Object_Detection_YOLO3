...
def compute_loss(opt, p, targets, model, giou_flag=True):  # predictions, targets, model
    # print("p: ", p)
    # ft = torch.cuda.FloatTensor(device=opt.device) if p[0].is_cuda else torch.Tensor
    # ft = torch.cuda.FloatTensor.to("cpu").to(opt.device) if p[0].is_cuda else torch.Tensor
    # print("ft: ", ft)
        ...
        # print("\n prediction lcls: \n", lcls)
        # print(" prediction lbox: \n", lbox)
        # print(" prediction lobj: \n", lobj)

    tcls, tbox, indices, anchor_vec = build_targets(model, targets)
    # print("\n target lcls: \n", tcls[1])
    # print(" target lbox: \n", tbox[1].shape)
    # print(" target indices: \n", indices[1].shape)
    # print(" target anchor_vec: \n", anchor_vec[0].shape)
    ...
    if p[0].is_cuda:
        lcls = torch.cuda.FloatTensor([0], device=opt.device)
        lbox = torch.cuda.FloatTensor([0], device=opt.device)
        lobj = torch.cuda.FloatTensor([0], device=opt.device)

        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.cuda.FloatTensor([h['cls_pw']], device=opt.device),
                                      reduction=red)
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.cuda.FloatTensor([h['obj_pw']], device=opt.device),
                                      reduction=red)
